...
class resnet_v1_101_feature_flow(Symbol):
    ...
    def get_train_feature_distance_symbol(self, cfg):
        data = mx.sym.Variable(name="data")
        data_ref = mx.sym.Variable(name="data_ref")

        concat_data = mx.sym.Concat(name='concat_data', *[data_ref, data], dim=0)
        concat_feat = self.get_resnet_v1(concat_data)
        concat_feat_stop_grad = mx.sym.BlockGrad(concat_feat)
        conv_feats = mx.sym.SliceChannel(concat_feat_stop_grad, axis=0, num_outputs=2)

        flow, scale_map = self.get_flownet(data, data_ref)
        flow_grid = mx.sym.GridGenerator(data=flow, transform_type='warp', name='flow_grid')
        warp_conv_feat = mx.sym.BilinearSampler(data=conv_feats[0], grid=flow_grid, name='warping_feat')
        warp_conv_feat = warp_conv_feat * scale_map

        L2_feature_loss_ = mx.sym.mean(mx.sym.square(conv_feats[1] - warp_conv_feat))
        L2_feature_loss = mx.sym.MakeLoss(name='L2_feature_loss', data=L2_feature_loss_, grad_scale=1.0)

        self.sym = L2_feature_loss
        return L2_feature_loss
