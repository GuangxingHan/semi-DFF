...
class TestLoader(mx.io.DataIter):
    def __init__(self, feat_sym, roidb, config, batch_size=1, shuffle=False,
                 has_rpn=False):
    ...
    def get_batch(self):
        ...
        if self.key_frameid == self.cur_frameid: # key frame
            ...
            data_shape['data_key'] = self.data_key.shape
            data_shape['feat_key'] = np.zeros((1,self.cfg.network.DFF_FEAT_DIM,1,1)).shape
            self.feat_sym.infer_shape(data_shape)
            self.feat_key_shape = self.feat_sym.out_shape_dict['feat_conv_3x3_relu_output']
        ...
        extend_data = [{'data': data[0]['data'],
                        ...
                        'feat_key': np.zeros(self.feat_key_shape)}]
