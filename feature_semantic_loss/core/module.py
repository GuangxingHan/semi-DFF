    ...
    def update(self):
        ...
        if self._update_on_kvstore:
            _update_params_on_kvstore(self._exec_group.param_arrays,
                                      ...
                                      self._kvstore,
                                      param_names=self._param_names)  # 2017-11-1
        else:
...
class MutableModule(BaseModule):
    ...
    def set_params(self, arg_params, aux_params, allow_missing=False, force_init=True):   # 2017-11-18
    ...
    def bind(self, data_shapes, label_shapes=None, for_training=True,
        ...
        if label_shapes != None and label_shapes.count(None) != len(label_shapes):  # if not label_shapes.count(None) == len(label_shapes): 2017-10-26
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            for nbatch, data_batch in enumerate(train_data):
                ...
                print len(data_batch.data), data_batch.data[0][0]
                # img = np.transpose(data_batch.data[0][0].asnumpy()[0,:,:,:], (1,2,0))
                PIXEL_MEANS = np.array([103.06, 115.90, 123.15])
                img = transform_inverse(data_batch.data[0][0].asnumpy(), PIXEL_MEANS)
                # img_ref = np.transpose(data_batch.data[0][1].asnumpy()[0,:,:,:], (1,2,0))
                img_ref = transform_inverse(data_batch.data[0][1].asnumpy(), PIXEL_MEANS)
                cv.imshow("img", img)
    ...
    def forward(self, data_batch, is_train=None):
        ...
        # get input_shapes
        if is_train and data_batch.provide_label != None and data_batch.provide_label.count(None) != len(data_batch.provide_label):  # if is_train: 2017-10-26
