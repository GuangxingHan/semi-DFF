...
from utils.image import get_pair_image
...
class ImagenetVIDIter(mx.io.DataIter):
    ...
    def parfetch(self, sub_imagedb):
        ...
        imgs, ref_imgs, eq_flags, roidb = get_pair_image(sub_imagedb, self.cfg)
        im_array = imgs[0]
        ref_im_array = ref_imgs[0]
        eq_flag_array = np.array([eq_flags[0],], dtype=np.float32)
        im_info = np.array([roidb[0]['im_info']], dtype=np.float32)

        data = {'data': im_array, 'data_ref': ref_im_array, 'eq_flag': eq_flag_array, 'im_info': im_info}
...
def train_net(args, ctx, pretrained, pretrained_epoch, pretrained_flow, pretrained_flow_epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    if config.TRAIN.RESUME:
        ...
        arg_params, aux_params = load_param(pretrained, pretrained_epoch, convert=True)
        if config.network.with_pretrained_flow:
            arg_params_flow, aux_params_flow = load_param(pretrained_flow, pretrained_flow_epoch, convert=True)
    ...
    # metric
    L2_rpn_cls_loss = metric.L2_RPN_cls_LossMetric(config)
    L2_rpn_bbox_loss = metric.L2_RPN_bbox_LossMetric(config)
    L2_cls_loss = metric.L2_cls_LossMetric(config)
    L2_bbox_loss = metric.L2_bbos_LossMetric(config)
    eval_metrics = mx.metric.CompositeEvalMetric()
    eval_metrics.add(L2_rpn_cls_loss)
    eval_metrics.add(L2_rpn_bbox_loss)
    eval_metrics.add(L2_cls_loss)
    eval_metrics.add(L2_bbox_loss)
...
def main():
    ...
    train_net(args, ctx, config.network.pretrained, config.network.pretrained_epoch, config.network.pretrained_flow, config.network.pretrained_flow_epoch, config.TRAIN.model_prefix,
